  ...
  const tasks = scenes.map(async (scene) => {
    ...
    try {
      if (scene.type === 'ai_video') {
        ...
        console.log(`[generateAssets]   scene ${scene.scene_no}: reference_image="${scene.reference_image || 'N/A'}", refImage=${refImage ? (refImage.startsWith('data:') ? 'base64' : 'URL') : 'none'}`)
        ...
        console.log(`[generateAssets]   scene ${scene.scene_no}: source="${scene.source}", found=${!!imageUrl}`)
        if (!imageUrl) {
        ...
        if (imageUrl) {
          ...
          console.log(`[generateAssets]   scene ${scene.scene_no}: motion_image → Kling i2v`)
          try {
            const videoResult = await callFalAI('fal-ai/kling-video/v2.5/turbo/image-to-video', {
              prompt: motionPrompt,
              image_url: imageUrl,
