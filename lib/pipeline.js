...
export async function generateAssets(scenes, inputImages) {
  ...
  const tasks = scenes.map(async (scene) => {
    try {
      if (scene.type === 'ai_video') {
        ...
        if (sourceUrl) {
          assets[scene.scene_no] = sourceUrl
        } else {
      ...
      try {
        ...
        // 유효한 소스 이미지라도 있으면 사용
        if (firstValidImage) assets[scene.scene_no] = firstValidImage
      }
    }
  })
  ...
  console.log(`[generateAssets] result: ${filled}/${total} scenes have valid assets`)
