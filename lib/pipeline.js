    ...
    try {
      if (scene.type === 'ai_video') {
        ...
        if (generatedImageUrl) {
          ...
          console.log(`[generateAssets] scene ${scene.scene_no}: generated_image → i2v (${motionPrompt.slice(0, 50)}...)`)
          try {
            ...
            assets[scene.scene_no] = videoResult.video?.url || videoResult.output?.url || generatedImageUrl
          } catch (i2vErr) {
            console.warn(`[generateAssets] scene ${scene.scene_no}: i2v failed, using static image:`, i2vErr.message)
        ...
        let imageUrl = validImages[scene.source]
        if (!imageUrl) {
          // 소스 이미지가 없으면 FLUX로 생성
          console.log(`[generateAssets] scene ${scene.scene_no}: source "${scene.source}" not found, generating image first`)
        ...
        if (imageUrl) {
          // image-to-video로 모션 영상 생성
          const motionPrompt = scene.prompt || scene.text_overlay || `Product shot with subtle ${scene.motion || 'slow_zoom_in'} motion, professional lighting, 9:16 vertical`
          console.log(`[generateAssets] scene ${scene.scene_no}: motion_image → image-to-video (${motionPrompt.slice(0, 50)}...)`)
          try {
            const videoResult = await callFalAI('fal-ai/kling-video/v2.5/turbo/image-to-video', {
              prompt: motionPrompt,
              image_url: imageUrl,
            ...
            assets[scene.scene_no] = videoResult.video?.url || videoResult.output?.url || imageUrl
          } catch (i2vErr) {
            console.warn(`[generateAssets] scene ${scene.scene_no}: i2v failed, using static image:`, i2vErr.message)
