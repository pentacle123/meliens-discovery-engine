...
export function convertToRenderScript(storyboard, assets, narrationUrl, bgmUrl) {
  ...
  for (const scene of storyboard.scenes) {
    ...
    // 모든 씬 타입이 이제 AI 영상을 생성하므로, URL로 video/image 판별
    const isVideo = assetUrl.includes('.mp4') || assetUrl.includes('.webm') || assetUrl.includes('/video')
    if (isVideo) {
      elements.push({
        type: 'video',
        source: assetUrl,
