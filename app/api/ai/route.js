...
- 시즌 문화: 수능/졸업/입사 시즌, 명절 선물, 빼빼로데이, 화이트데이 등 한국 고유 이벤트
- 절대 비현실적인 가격이나 상황을 사용하지 마세요. 한국 소비자가 즉시 공감할 수 있는 현실적인 시나리오만 작성하세요.

다음 제품에 대해 구매 전환 확률이 높은 "날카로운 맥락 조합" TOP 10을 JSON으로 생성하세요.
...
═══ 이중 방향 사고 (Dual-Direction Thinking) — 필수 ═══
아이디어를 생성할 때 반드시 두 가지 방향으로 사고하세요:

방향 1 — 소비자 기점 (Forward):
WHO(타겟) → PAIN(불편) → 이 제품이 왜 필요한지 → 아이디어
예: '니트 좋아하는 30대 직장인' → '보풀 때문에 옷이 낡아 보임' → '보풀제거기로 새 옷처럼'

방향 2 — 제품 기점 (Reverse):
제품의 고유 FEATURE(특성/스펙) → 이 특성이 해결하는 구체적 문제는? → 이 문제를 가장 절실히 느끼는 사람은 누구? → 아이디어
예: '맥세이프 거치대의 강한 자력' → '무거운 폰도 안 떨어짐' → '아이폰 16 프로맥스 사용자(227g)' → '프로맥스 유저인데 아직 흡착식 거치대 쓰세요?'

═══ 확장 맥락 축 (기존 6축에 추가) ═══
기존 6축(WHO/WHEN/WHERE/PAIN/NEED/INTEREST)에 더해 다음 확장 축도 적극 고려하세요:
- DEVICE: 사용하는 디바이스/생태계 (아이폰 프로맥스, Z플립, 갤럭시, 맥북 등)
- TRIGGER: 구매를 촉발하는 생애 이벤트 (신차 출고, 폰 교체, 이사, 여행 예약, 졸업, 입사 등)
- BARRIER: 구매를 막는 우려/걱정 (부작용, 옷감 손상, 떨어짐, 가격 대비 효과 등)
- SWITCH_FROM: 현재 쓰고 있는 대안 (다이소 제품, 경쟁사 제품, 이전 방식 등)
- VEHICLE_ENV: 구체적 사용 환경 (차종, 방 크기, 피부 타입, 모발 타입 등)

═══ 4단계 아이디어 도출 프로세스 ═══
반드시 아래 4단계를 순서대로 따르세요:

STEP 1 — Forward(소비자 기점) 아이디어 생성 (4개):
소비자의 WHO/PAIN/WHEN/WHERE에서 출발하여 아이디어 4개를 생성하세요.
- 소비자 검색 여정(search_journey)을 분석하여 전환 경로를 반영
- 소비자 우려사항(consumer_concerns)을 숏폼 후킹 카피와 페인포인트에 직접 활용
- 콘텐츠 인사이트(content_insight)를 아이디어 방향성의 근거로 사용

STEP 2 — Reverse(제품 기점) 아이디어 생성 (3개):
이 제품만의 고유 FEATURE/스펙에서 출발하여 아이디어 3개를 생성하세요.
- 제품의 핵심 강점 각각에서 역으로 "이 기능이 가장 절실한 사람은 누구?" 사고
- 스펙 숫자를 직접 활용 (예: "12,000회 진동", "2,500mAh", "74g 초경량" 등)
- 경쟁 제품과의 차별점에서 출발하는 아이디어 포함

STEP 3 — Extended(확장 축) 아이디어 생성 (3개):
확장 축(DEVICE/TRIGGER/BARRIER/SWITCH_FROM/VEHICLE_ENV)을 활용하여 아이디어 3개를 생성하세요.
- 경쟁 환경(competitive_landscape)을 비교형 숏폼 아이디어에 활용
- 구매 촉발 이벤트(TRIGGER)와 제품을 연결
- 구매 장벽(BARRIER)을 정면 돌파하는 콘텐츠

STEP 4 — 검색 데이터 & VOC 교차 검증:
...
═══ 3단계 아이디어 티어 시스템 (필수) ═══
10개 조합을 반드시 아래 3개 티어로 나누세요:

TIER 1 — "검증된 안전 조합" (rank 1~4):
- 검색 데이터와 VOC 근거가 가장 강한 조합
- 이미 소비자가 검색하고 후기를 남기는 검증된 수요
- data_evidence: 🟢 위주
- 이 티어의 tier값: "safe"

TIER 2 — "크로스 카테고리 조합" (rank 5~7):
...
═══ 핵심 원칙 ═══
1. 6개 축 + 확장 축을 전부 연결하지 마세요. 2~3축만 날카롭게.
2. 매우 구체적이고 현실적인 상황을 묘사하세요. 추상적이면 안 됩니다.
3. 각 아이디어에 hook_copy를 반드시 포함하세요 — 숏폼 첫 3초에 사용할 후킹 카피 한 줄.
4. 10개 조합이 서로 겹치지 않아야 합니다. 각각 완전히 다른 관점이어야 합니다.
5. data_evidence는 STEP 4의 검증 결과 이모지(🟢/🟡/🔵)로 시작하고, 실제 검색량 수치를 포함하세요.
...
7. ai_producible: 이 아이디어를 간단한 제품 촬영(스튜디오, 탁상)만으로 제작 가능한지 판단하세요.
   ...
   - false: 실제 사람 연기, 특정 장소 로케이션, 실험/시연 등 복잡한 촬영 필요
...
9. thinking_direction 필드를 반드시 포함하세요:
   - "forward": 소비자 기점 (STEP 1에서 생성한 아이디어)
   - "reverse": 제품 기점 (STEP 2에서 생성한 아이디어)
...
- thinking_direction은 반드시 "forward", "reverse", "extended" 중 하나여야 합니다.
...
당신은 한국 시장에 정통합니다. 한국의 실제 물가, 소비 문화, 유행어, 생활 패턴을 정확히 반영하세요.
- 가격 감각: 카페 커피 5,000원, 점심 만원대, 자취 월세 50~80만원, 신차 3,000만원~1억원+
- 소비자 언어: "가성비", "갓생", "꿀팁", "핵이득", "인생템" 등 실제 한국 MZ세대 표현
- 유통 채널: 쿠팡, 네이버 쇼핑, 올리브영, 다이소 등 한국 실제 채널
- 비현실적인 가격/상황 금지. 한국 소비자가 즉시 공감할 수 있는 현실적인 시나리오만 작성.
